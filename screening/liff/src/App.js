...
export function toRelative(date) {
  if (DateTime.fromMillis(date).diffNow().as('minutes') > -1) return 'เมื่อสักครู่';
...
export default function App() {
  ...
  useEffect(() => {
    const faviconUpdate = async () => {
      ...
      if (!pass) {
        favicon.href = "assets/images/redcircle.png";
        console.log(favicon);
      }
      //if above 0, we set back to green
      else {
        favicon.href = "favicon.ico" //"assets/images/greencircle.png";
        console.log(favicon);
      }
    };
  ...
  return (<React.Fragment>
    ...
    <Box sx={{ display: 'flex', flexGrow: 1 }}>
      <CssBaseline />
      <AppBar position="fixed" open={open} sx={{ top: 'auto', bottom: {xs: 0, sm :'auto'}}}>
        <Toolbar>
          <IconButton
            color="inherit"
